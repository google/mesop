/**
 * This code forked from: https://github.com/mpetazzoni/sse.js/blob/538c69f5e79d58a92f81a7f437d203cbb391999d/lib/sse.js
 *
...
class SSE {
  ...
  dispatchEvent(event: CustomEvent | null): boolean {
    if (!event) {
      return true;
    }
    ...
    if (this.hasOwnProperty(onHandler)) {
      (this as any)[onHandler].call(this, event);
  ...
  private _onStreamFailure(e: ProgressEvent): void {
    ...
    (event as any).data = (e.currentTarget as any).response;
    this.dispatchEvent(event);
    this.close();
  }
  ...
  private _parseEventChunk(chunk: string): CustomEvent | null {
    ...
    const e: {
      ...
      data: string | null;
      event: string | null;
    } = {id: null, retry: null, data: null, event: null};
    chunk.split(/\n|\r\n|\r/).forEach((line) => {
